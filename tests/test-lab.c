...
void test_buddy_realloc(void){
  fprintf(stderr, "->Testing buddy_realloc function\n");
  ...
  for(size_t i = 0; i < initial_size; i++){
    ((char *)ptr)[i] = (char)(i & 0XFF); //data fill
  }

  size_t new_size = 128; //bigger size
  void *new_ptr = buddy_realloc(&pool, ptr, new_size);
  assert(new_ptr != NULL);
  assert(new_ptr != ptr);

  for(size_t i = 0; i < size; i++){
    assert(((char * )new_ptr)[i] == (char)(i & 0xFF));
  }

  size_t smaller_size = 32; //smaller size
  void *shrunk_ptr = buddy_realloc(&pool, new_ptr, smaller_size);
  assert(shrunk_ptr != NULL);

  for(size_t i = 0; i < smaller_size; i++){
    assert(((char *)shrunk_ptr)[i] == (char)(i & 0xFF));
  }

  size_t malloc_size = 64;
  void *malloc_ptr = buddy_realloc(&pool, NULL, malloc_size); //realloc with null ptr
  assert(malloc_ptr != NULL);

  void *freed_ptr = buddy_realloc(&pool, malloc_ptr, 0); //realloc with size 0
  assert(freed_ptr == NULL);


  buddy_free(&pool, shrunk_ptr);
  buddy_destroy(&pool);
}
